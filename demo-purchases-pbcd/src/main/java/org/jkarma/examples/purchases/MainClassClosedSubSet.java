...
public class MainClassClosedSubSet {
...
//	}
    ...
    public static void main(String[] args) throws IOException {
        ...
        final MainClassClosedSubSet mainClass = new MainClassClosedSubSet();
        ...
        try {
            ...
            dataset.forEach(transaction -> {
                ...
                try {
                    float num = 0;
                    float den = 0;
                    HashSet<String> tItems = Sets.newHashSet(transaction.getItems());
                    Set<Set<String>> subSets = Sets.powerSet(tItems);
                    for(Set<String> set : subSets)
                    {
                        for (Pattern<String, ProjectedDB<String>> p : detector.getLattice()) //IllegalArgumentException finche' non raggiunge il numero di transazioni pari a blockSize
                        {
                            Set<String> valori = new HashSet<>();
                            addValue(valori, p.getItemSet());
                            if(valori.containsAll(set))
                            {
                                num++;
                            }else{
